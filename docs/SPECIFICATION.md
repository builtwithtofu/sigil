...
- `atomic=true` requests replacement only after successful production, including empty output. Local POSIX regular files use private same-directory staging and rename. Failure or cancellation observed before the final publication decision removes unpublished staging. Once publication begins, cancellation cannot guarantee that the previous destination remains: rename is not cancellable, and published output is not rolled back. Each retry attempt starts afresh.
...
- Isolation wrappers with unrestricted filesystems (`@isolated`, network loopback, and memory lock) forward file output to their parent session. Filesystem, privilege-drop, and sandbox wrappers currently lack policy-preserving output support and fail closed rather than writing outside their restrictions.
